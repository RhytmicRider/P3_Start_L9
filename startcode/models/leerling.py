...
class Leerling(Karakter):
    ...
    def beschrijf(self) -> str:
        ...
        return f"{basis} - Leerling {self.klas}"
    ...
    def verwerk_bericht(self, bericht: str, speler_inventory: list) -> str:
        ...
        """
        # TODO Oefening 3: Implementeer steen-papier-schaar als self.spel == "steen-papier-schaar"
        if self.spel == "steen-papier-schaar":
            opties = ["steen", "papier", "schaar"]

            keuze = bericht.lower().strip()

            if keuze not in opties:
                return f"{self.naam}: type steen, papier of schaar"
